...
var dialogStr = dialog_get(), newSpr, speaker, colDefault, i;
//Default text drawing colour   ---------------------------
colDefault = make_color_rgb(255, 236, 181);
...
if(sprite[0, 0] != noone) {
    ...
    draw_sprite_ext(sprite[0, 0], sprite[0, 1], sprite[0, 2], window_get_height() - DIAHEIGHT, 1, 1, 0, c_white, 1);
...
if(sprite[2, 0] != noone) {
    ...
    if(sprite[2, 2] > 0) {
        //Move the other way
        draw_sprite_ext(sprite[2, 0], sprite[2, 1], sprite[2, 2], window_get_height() - DIAHEIGHT, 1, 1, 0, c_white, 1);
...
if(sprite[1, 0] != noone) {
    ...
    draw_sprite_ext(sprite[1, 0], sprite[1, 1], window_get_width() - sprite[1, 2], window_get_height() - DIAHEIGHT, -1, 1, 0, c_white, 1);
...
if(sprite[3, 0] != noone) {
    ...
    if(sprite[3, 2] > 0) {
        //Move the other way
        draw_sprite_ext(sprite[3, 0], sprite[3, 1], window_get_width() - sprite[3, 2], window_get_height() - DIAHEIGHT, -1, 1, 0, c_white, 1);
...
draw_rectangle(boxX, boxY, boxX + (DIAWIDTH * window_get_width()), boxY + DIAHEIGHT, false);
//Separator between speaker and the message ---------------
draw_set_color(make_color_rgb(255, 236, 181));
draw_line(boxX, boxY + DIASPKRH, boxX + (DIAWIDTH * window_get_width()), boxY + DIASPKRH);
...
//Get the speaker, and remove it, as well as the 'SPKR' character, from the dialogStr
            ...
            if(colChange) {
                switch(curChar) {
                    case 'k':   //key
                        ...
                        break;
                    case 'l':   //location
                        draw_set_color(make_color_rgb(200, 200, 250));
                        break;
                    case 'a':   //animal
                        draw_set_color(make_color_rgb(220, 100, 250));
                        break;
                    /* Add more colours -------------------
                    case 'SYMBOL':
...
//        audio_play_sound(aDialog, 0, false);
    }
    ...
    for(i = minOption; i <= maxOption; i++) {
        if(i == currentOption) {
            ...
            draw_set_alpha(0.2);
            draw_rectangle(boxX, boxY + DIASPKRH + DIAPADV + (i - minOption) * string_height("A"), boxX + (DIAWIDTH * window_get_width()), boxY + DIASPKRH + DIAPADV + (i - minOption + 1) * string_height("A"), false);
        ...
        if(minOption != 0) {
            ...
            //Change if you want    -----------------------
            draw_triangle(  boxX + (DIAWIDTH * window_get_width()) - 16, boxY + DIASPKRH + DIAPADV,
                            boxX + (DIAWIDTH * window_get_width()) - 16 - string_height("A") / 2, boxY + DIASPKRH + DIAPADV + string_height("A"),
                            boxX + (DIAWIDTH * window_get_width()) - 16 + string_height("A") / 2, boxY + DIASPKRH + DIAPADV + string_height("A"), false);
        } if(maxOption != optionCount - 1) {
            //Draw downward triangle to show there are more options below
            //Change if you want    -----------------------
            draw_triangle(  boxX + (DIAWIDTH * window_get_width()) - 16, boxY + DIASPKRH + DIAPADV + ((optionsToDraw) * string_height("A")),
                            boxX + (DIAWIDTH * window_get_width()) - 16 - string_height("A") / 2, boxY + DIASPKRH + DIAPADV + ((optionsToDraw - 1) * string_height("A")),
                            boxX + (DIAWIDTH * window_get_width()) - 16 + string_height("A") / 2, boxY + DIASPKRH + DIAPADV + ((optionsToDraw - 1) * string_height("A")), false);
        }
    }
